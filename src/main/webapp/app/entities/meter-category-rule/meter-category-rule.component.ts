...
@Component({
    selector: 'jhi-meter-category-rule',
    templateUrl: './meter-category-rule.component.html'
})
export class MeterCategoryRuleComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/meter-category-rule'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/meter-category-rule', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: MeterCategoryRule) {
        return item.id;
    }
    registerChangeInMeterCategoryRules() {
        this.eventSubscriber = this.eventManager.subscribe('meterCategoryRuleListModification', (response) => this.loadAll());
    }
