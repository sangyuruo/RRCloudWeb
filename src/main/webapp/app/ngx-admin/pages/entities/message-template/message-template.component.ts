...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/message-template'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: MessageTemplate) {
        return item.id;
    }
    registerChangeInMessageTemplates() {
        this.eventSubscriber = this.eventManager.subscribe('messageTemplateListModification', (response) => this.loadAll());
    }
